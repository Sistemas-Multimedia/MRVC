...
inverse MCDWT, if $I=3$ (the GOP size $G=8$, i.e. $4$ temporal resolution levels (TRLs)), the
images of the second GOP (GOP 1, considered the first GOP, GOP 0, only
...
predictor=1
python3 -O MDWT.py  -p /tmp/
python3 -O MCDWT.py -p /tmp/     -P $predictor -I 1
...
python3 -O MCDWT.py -b -p /tmp/ -N 9 -P $predictor -I 2
...
python3 -O MCDWT.py -p /tmp/     -P $predictor -b -I 2
python3 -O MDWT.py  -p /tmp/                   -b
...
\subsection*{Example: Quantization of all $H$ subbands}
\begin{verbatim}
predictor=2
