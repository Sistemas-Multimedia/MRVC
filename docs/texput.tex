...
\begin{itemize}
\item Subband $L_2$ with the scale $2$ of the image. If such image is
  $I$, we will denote the scale $2$ as $I.L_2$.
\begin{verbatim}
    +------+
    |      |
Y/4 |  L_2 |
...
Fig~\ref{fig:MDWT}. The 1-iteration MDWT inputs a sequence of
frames $\{F_i\}$ and outputs a sequence of decompositions
$\{\{F_i\}.L, \{F_i\}.H\}$, also called sequence spatial
subbands. Notice that $\{F_i\}.L$ represents the scale $1$ of the
sequence (a version of the original video $\{F_i\}$ but with a
resolution $Y/2\times X/2$), and that $\{F_i\}.L_0 = \{F_i\} =
