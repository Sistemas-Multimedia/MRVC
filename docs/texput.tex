...
\href{https://vicente-gonzalez-ruiz.github.io/image_transformations_for_coding/#x1-3100020}{The
  2D DWT (Distrete Wavelet Transform)} is a digital transform that,
applied to an image, performs a spatial (2D) decorrelation and obtains
a
...
implements the forward and backward 1-iteration 2D DWT for color
images. In the rest of this document, only the 1-iteration 2D DWT
(also known as the 2D DWT step) will be used to implement the
$N$-iterations 2D DWT. This can be done by appliyin the 1-iteration 2D
DWT recursively to the $LL$ subband. We will refeer to the 1-iteration
2D DWT as only as the DWT, for the sake of simplicity.
