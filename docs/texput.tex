...
$\{F_i.L_1\}$ and $\{F_i.H_1\}$, providing the SRLs $\{F_i.L_1\}$ and
$\{F_i.L_0\}=\{F_i\}$. When we refeer to the whole frame sequence, we
also use the notation $\{F_i\}=\{L_0\}$ and $\{F_i.L_1\}=\{L_1\}$.
...
in a ``t+2D'' transform we can apply directly any frame predictor
...
and it is applied first. Basically, the ``t'' stage is a
$T$-iterations 1D
...
A special feature of MCOLP is that the motion information is
...
\begin{figure}
  \centering
  \myfig{graphics/temporal_decorrelation}{10cm}{1000}
...
\subsection*{Example: 1-iteration MDWT + 1-iteration MCOLP (\texttt{PRMC(dwt\_iters=1, olp\_iters=1)})}
...
\subsection*{Example: 1-iteration MDWT + 2-iterations MCOLP (\texttt{PRMC(dwt\_iters=1, olp\_iters=2)})}
...
\subsection*{Example: 1-iteration MDWT + 4-iterations MCOLP (\texttt{PRMC(dwt\_iters=1, olp\_iters=4)})}
