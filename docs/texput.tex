...
\subsection{Multi-levels 2D-DWT structures}
...
spatial resolution levels.}) 2D-DWT applied to the $LL$ subband. For example, if
the 2D-DWT is applied $k$-times to the $LL$ subband, recursively, a
decomposition of spatial resolution $k+1$-levels is generated. For the
sake of simplicity, we will denote the subbands $\{LH^k, HL^k, HH^k\}$
as only $H^k$, and $LL^k$ as only $L^k$.

\subsection{Scalability provided by the 2D-DWT}
Depending on the number of subbands (pyramid levels in the pyramid
representation) that we use, we can reconstruct an image at different
spatial scales (resolutions). For example, if an image $s_1$ has been
...
\subsection{A 2D-DWT 1-levels example}
\begin{verbatim}
