...
\item In order to implement a lossless system, the t stage must be
  carried out at full (scale 0) resolution, as minimum.
...
\item We can use the full (scale 0) resolution version of the motion
  ...
  is processed. Therefore, depending on the resolution of the scale 0
  and the computational resources (specially at the receiver side),
  this could be feasible or not.
...
texture information in a code-stream that can be decoded following
different orders, depending on the type of used scalability and the
avaliable bandwidth.

% Subband Motion Compensated Temporal Filtering (SMCTF) 
\section{Motion Compensated Discrete Wavelet Transform (MCDWT)}
MCDWT is a 2D+t transform. The 2D stage is MDWT. The t stage is a 1D
DWT, which removes the temporal redundancy between adjacent $H$
