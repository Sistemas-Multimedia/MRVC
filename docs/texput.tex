...
\item Usually, a \textbf{
  ...
  ratio}.
\item The transform domain provides a priorization of the information
  contained in the signal, enabling a
  \textbf{\href{https://en.wikipedia.org/wiki/Image_resolution}{multiresolution}
    representation (spatial and temporal)} of the visual information
  (spatial
  \href{http://inst.eecs.berkeley.edu/~ee290t/sp04/lectures/videowavelet_UCB1-3.pdf}{scalability}),
  as well as an energy compactation in a small number of coefficients.
\end{enumerate}

\section{The DWT (Discrete Wavelet Transform)}
See \href{https://vicente-gonzalez-ruiz.github.io/image_transformations_for_coding/#x1-3100020}{image transformations for coding}.
...
(2D) decorrelation and obtains a
...
equivalence between 2D DWT subbands (a \emph{(subband) decomposition})
and
...
implements the forward and backward 2D DWT for color images.

\lstinputlisting[firstline=9, lastline=105, language=python, caption=DWT.py]{../src/DWT.py}

$L$ and $H$ stands for \emph{low-pass filtered} and \emph{high-pass
  filtered}, respectively.  More information about the implementation
can be found at
\href{https://pywavelets.readthedocs.io/en/latest/index.html}{PyWavelets}.

\subsection{Multi-level 2D DWT structures}
A dyadic 2D DWT is a recursive use of the
...
in time. Spatial scalability is a direct consequence of the 2D DWT. On
the other hand, it is trivial to observe that MDWT provides
\emph{fully} temporal scalability (we can access to the images
randomly) because each image of the input sequence is transformed
independently.
...
\href{http://www.polyvalens.com/blog/wavelets/theory}{shift
  invariant}, which basically means that a even an (integer) pixel
accuracy movement of an object that is shown by two different images
will generate different wavelet coefficients.  Therefore, motion
estimators which compare coefficient values will not work on the
decomposition domain. On the other hand, if we want to provide true
spatial scalability (processing only those
...
\item We can use the full (scale 0) resolution version of the motion
  ...
  increase unnecessarily when a low spatial resolution of the sequence
  is decoded. Therefore, depending on the resolution of the scale 0
  and the computational resources (specially at the receiver side),
  this could be affordable or not.
