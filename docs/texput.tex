...
decomposition of spatial resolution $(k+1)$ levels (SRLs) is
generated. For the sake of simplicity, we will denote the spatial
frame\footnote{``Frame'' is the name that most video coding standards
  use for reffering to a single image of the video.} subbands
$\{F.LH_k, F.HL_k, F.HH_k\}$ as only $F.H_k$, and $F.LL_k$ as only
$F.L_k$, where $F$ is a frame. See
...
$\{F_i.L_1\}$ and $\{F_i.H_1\}$ (sequence subbands), providing the
SRLs $\{F_i.L_1\}$ and $\{F_i.L_0\}=\{F_i\}$. For the sake of
simplicity, we will denote sequence spatial subbands $\{F_i.L_1\}=L_1$
and $\{F_i.H_1\}=H_1$, and the sequence spatial resolution levels as
$L_1$ and $L_0$.
...
We can think of MCLOPT such as a MCTF that is applied to a given
spatial resolution level, and because the motion is estimated using
the previous lower resolution level (which is available at the
decoder), it is not necessary to send the motion information in the
code-stream. Notice also that this process can be applied for any
...
\begin{figure}
  ...
  \caption{Subbands generated after running $1\times \mathtt{PRMC(iters=1)}$.}
  \label{fig:1xPRMC1}
\end{figure}
