...
\item [4.] For each coefficients $x,y$ in $[\hat{b.L}].H$:
  \begin{itemize}
  \item [a.] if $\hat{b.H}_{x,y}\leq\lambda$ send bit 1, or 0
    otherwise. If it is the first time that a 1 has been sent, send
    also its sign previously.% Ojo, no descarta bits transmitidos
  \end{itemize}
\item [5.] $\labmda /= 2$.
\item [6.] Goto step 4.
\end{itemize}
