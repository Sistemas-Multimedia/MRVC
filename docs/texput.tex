...
using a BLIC that cannot be exploited. The idea here is, is it
possible to predict the coordinates of the ones in a BP? If this is
possible, a sequence of bits (with so many bits as coefficients are in
$H$) can encode the success of the prediction. Suppose that
$\hat{H}\downarrow$ (the list of indexes the sorts $\hat{H}$ in
descending order by amplitude) determines the possible localization of
the most energetic coefficients in $H$ and in $\tilde{H}$. Therefore,
the MSB of the first elements of $\tilde{H}[\hat{H}\downarrow]$ (the
list of coefficients of the residue (approximately) sorted by energy)
will be 1 (with a high probability) and the rest will be 0 (with a
high probability). For example, if there are five 1's in the MSB of
the beginning of $\tilde{H}[\hat{H}\downarrow]$, the complete BP can
be encoded with the codewords <5><EOBP>. If for example, the MSB of the
first elements of $\tilde{H}[\hat{H}\downarrow]$ is 0101001000110...,
then the the output codewords are <0><1><1><1><2><1><3><2><EOBP>.


For example, if the prediction is that all the bits of the BP are 0 and this is true, the XOR of both predictions should b


only in a given coordinate of $LH$ there is a 1 (the rest of bits of the BP are 0)


If this is
true, the bits of a BP can be sorted and simple binary RLE (Run-Length
Encoding) can be used to encode efficiently the BP. For example, lets
suppose that there are only one 1 in the BP and we are able to predict
its coordidates in the BP. In that case, we only need to output a
codeword <1> to indicate that the position of the 1 is in the first
position of the sorted bits (supposing that the binary RLE starts
encoding runs of ones), and there is only one 1. If the prediction is
wrong but the 1 is placed in the second position of the sorted array,
the output is <0><1>. If there are two 1's and the prediction is
right, both are in the beggining of the sorted array. In this case the
output is <2>. If the prediction if wrong and the result of the
prediction is 010010..., the output is <0><1><2><1>.

By definition,
unpredictable information can not be compressed. Consecuently, the lossless encoding of $H$ is basically: (1) determine the 
