...
final coefficient prediction
\begin{equation}
  \hat{[b_b.H]}_{x,y} = \frac{[b_a.H]_{x,y}+[b_c.H]_{x,y}}{2},
\end{equation}
being $x,y$ the coordinate of the coefficient in the prediction
pyramid level. This simple computation, that can have very effective
for example in
\href{https://biteable.com/blog/tips/video-transitions-effects-examples/}{dissolves}
video transitions, can be counterproductive when objects appear only
in one of the reference images. For this reason, in this section a
different predictor is proposed, based on the prediction error
...
\begin{equation}
  ...
  \end{array}
\end{equation}
With this information, the improved coefficient prediction is define as
\begin{equation}
  \hat{[b_b.H]}_{x,y} = \frac{[b_a.H]_{x,y}[e_c.L]_{x,y}+[b_c.H]_{x,y}[e_a.L]_{x,y}}
      {[e_a.L]_{x,y}+[e_c.L]_{x,y}}.
\end{equation}

Notice that, if (for example) $[e_a.L]_{x,y}=0$ then
$\hat{[b_b.H]}_{x,y}=[b_a.H]_{x,y}$ and viceversa.
\begin{comment}
