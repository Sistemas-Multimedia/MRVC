...
A iteration of the MCDWT is carried out when the MCDWT butterfly is
applied to all the images of the input sequece using the pattern shown
in the Fig.~\ref{fig:MCDWT}.

\begin{figure}
  \centering
  \myfig{graphics/temporal_decorrelation}{12cm}{1200}
  \lstinputlisting[firstline=73, lastline=125, language=python, caption={Forward MCDWT (\href{https://github.com/Sistemas-Multimedia/MCDWT/blob/master/src/MCDWT.py}{MCDWT.py}).}]{../src/MCDWT.py}
  \lstinputlisting[firstline=127, lastline=179, language=python, caption={Backward MCDWT (\href{https://github.com/Sistemas-Multimedia/MCDWT/blob/master/src/MCDWT.py}{MCDWT.py}).}]{../src/MCDWT.py}
  \caption{Implementation of the MCDWT.}
  \label{fig:MCDWT}
\end{figure}

The forward MCDWT is the result of appliying one iteration of the
forward MCDWT recursively, to the image with number $2^i$, where $i$
is the iteration. Notice that if $N=2^I$ (where $N$ is the number of
images/decompositions and $I$ is the number of iterations), except for
the first and the last image of the sequence, all the $H$-subbands are
temporally decorrelated. Notice that the number of temporal escales is
$T=I+1$.
