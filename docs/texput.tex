...
\begin{figure}
  ...
  \caption{Implementation of the multicomponent 1-iteration DWT (forward and
    backward). More information about the
...
generating 3 SRLs: (1) $L_2$, (2) $L_1=\mathtt{iDWT}(L_2, H_2)$, and
(3) $L_0=\mathtt{iDWT}(L_1, H_1)$, where $\mathtt{iDWT}$ stands for
inverse DWT.
...
has been transformed using the DWT of 2 iterations, we get:
\begin{itemize}
\item Subband $L_2$ with the scale 2.
\begin{verbatim}
    +------+
    |      |
Y/4 |  L_2 |
    ...
    +------+
       X/4
\end{verbatim}
\item Subband $L_1\coloneqq L=\mathtt{iDWT}(L_2, H_2)$
  with the scale $1$.
\begin{verbatim}
    +-------------+
    |             |
...
\item $L_0=\mathtt{iDWT}(L, H)$ (the original image) with
  the scale $0$.
\begin{verbatim}
    +---------------------------+
    |                           |
...
\subsection*{Example: A 1-iteration DWT ($\mathtt{DWT(iters=1)}$)}
%{{{
...
\subsection*{Example: A 2-iterations DWT ($\mathtt{DWT(iters=2)}$)}
%{{{
...
Fig~\ref{fig:forward_MDWT}. The 1-iteration MDWT inputs a sequence of
frames $\{F_i\}$ and outputs a sequence of decompositions $\{F_i.L, F_i.H\}$, also called sequence spatial subbands. These subbands are the
result of decomposing each image $F_i$ into producing the SRLs
