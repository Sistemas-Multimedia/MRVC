...
the sake of simplicity, we will denote the subbands $\{LH_k, HL_k,
HH_k\}$ as only $H_k$, and $LL_k$ as only $L_k$. See
...
reconstruct a frame\footnote{``Frame'' is the name that most video
  coding standards use for reffering to a single image of the video.}
at different spatial scales (resolutions). For example, if a frame
...
The 2D-DWT can be applied to a sequence of frames by simply
transforming each frame of the sequence independently. This is done,
...
frames $\{s_i\}$ and outputs a sequence of decompositions $\{S_i\}$,
...
frames randomly) because each frame of the input sequence is
...
If we apply an inverse transform to the original frames, supposing
...
# Copy the current frames as LL subbands.
rm /tmp/*.png
for i in {0..4}; do
  frame_number=$(printf "%03d" $i)
  cp ../sequences/stockholm/$frame_number.png /tmp/LL$frame_number.png
done
...
# frames could appear brighter of darker). This is consequence of the
...
in a t+2D transform we can apply directly any frame predictor based on
...
\item The perfect reconstruction of the original frames only is
  ...
  reconstructed frames were smaller. This could be unfeasible for
...
applied first. Basically, the t stage is a $T$-iterations 1D
...
The predict step implemented by the MCOLP is performed in the
Orthogonal Laplacian Pyramid domain, where the coefficients have not
been subsampled as in the DWT domain. The advantage of using the OLP
domain over the DWT domain is that the OLP representation is
shift-invariant.
...
\href{https://en.wikipedia.org/wiki/Video_compression_picture_types}{bidirectionally
  predicted ``B'' frame}\footnote{$\tilde{b}$ can be considered a
  B-type frame. However, notice that only the high-frequency
  information of $\tilde{b}$ has been compensated with the
  high-freq. information of the neighbor frames. In a normal B frame,
  all the frequencies are compensated.}) and $c$ (another intra-coded
``I'' frame). This replacement is fully reversible because the forward
...
\begin{figure}
  ...
  \lstinputlisting[firstline=79, lastline=131, language=python, caption={MCOLP (\href{https://github.com/Sistemas-Multimedia/PRMC/blob/master/src/MCOLP.py}{MCOLP.py}).}]{../src/MCOLP.py}
  \lstinputlisting[firstline=133, lastline=174, language=python, caption={iMCOLP (\href{https://github.com/Sistemas-Multimedia/PRMC/blob/master/src/MCOLP.py}{MCOLP.py}).}]{../src/MCOLP.py}
...
the (1-iteration) MDWT, producing 2 SRLs by frame. Next, the $H$
...
\subsection*{Example: 1-iteration MDWT + 2-iterations MCOLP}
...
compensates also the $H$ subband of the frame 2. This example matches
...
dyadic access to the frames. For example, after applying each
iteration of the inverse MCOLP, if $T=3$ (the GOP size $G=8$, i.e. $4$
temporal resolution levels (TRLs) have been generated), the frames of
the second GOP (GOP 1, considering that the first GOP (GOP 0) is only
composed by the first frame of the sequence) should be reconstructed
in this order:
