...
\item [5.] $\lambda_0 = \lambda = 1 <<
  \text{int}(\log_2(\text{abs}(d_{0})))$ /* maximum amplitude */.
...
\item [3.] Encode de subband:
  \begin{equation}
    ...
    \end{array}
  \end{equation}
\item [4.] Create a linear array with the 3 matrices
  \begin{equation}
    H = [LH^2, HL^2, HH^2].
  \end{equation}
...
\item [6.] Find the indices for the $LH^2$, $HL^2$ and $HH^2$ matrices
  ...
  \begin{equation}
    ...
    \text{height}).
  \end{equation}
\item [7.] Go over $H'$ looking for the wavelet coefficients of
  subbands $LH^2$, $HL^2$ and $HH^2$ in descending order by energy
  \begin{equation}
    \begin{array}{l}
