...
After a $T$-levels MCDWT transform, I-type images are decomposed into
...
MSBP), sending before the corresponding sign. If more than one if
...
high-frequency coefficients, and therefore to sort them by their
magnitude. The idea is to predict the $H$ coefficients by introducing
some (visual) information in the $[L]$ subband, and to perform again
the 2D-DWT (using the same wavelets). This generates a $\hat{H}$
subbands that substracted to $H$ should remove the visual information
from them (decorrelating the signs and reducing the number of bits
necessary to represent most of the coefficients). Moreover, the
coefficients of $\hat{H}$ sorted by magnitude in descending order can
be used to estimate which bits inside of each bit-plane should be
encoded first.
...
\item Sort $\hat{H}$ by magnitude in descending order.
  \begin{equation}
...
\item Go over $\hat{H}\downarrow$, sending by bit-planes the
  ...
  \begin{equation}
    \begin{array}{l}
      \text{for~}b\text{~in} log ...
      \text{for~}i\text{~in} \hat{H}\downarrow: \\
      %~ 
