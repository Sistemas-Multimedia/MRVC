\documentclass[tikz,14pt,border=10pt]{standalone}
\usepackage{amsmath}
\usepackage{textcomp}
\usetikzlibrary{shapes,arrows}
\usetikzlibrary{calc}

\begin{document}
% Definition of blocks:
\tikzset{%
  block/.style    = {draw, rectangle, minimum height = 2cm,
    minimum width = 2cm, node distance = 3.5cm},
  sum/.style      = {draw, circle, node distance = 3.5cm}, % Adder
  input/.style    = {coordinate}, % Input
  output/.style   = {coordinate} % Output
}
% Defining string as labels of certain blocks.
\newcommand{\suma}{\Large$+$}
\newcommand{\inte}{$\displaystyle \int$}
\newcommand{\derv}{\huge$\frac{d}{dt}$}
\newcommand{\DWT}[2]{\begin{tikzpicture}%
    \draw node [draw, minimum width = 1cm, minimum height = 1cm] (0) {$#1$};
    \draw node [draw, minimum width = 1cm, minimum height = 1cm, node distance = 1cm, right of=0, fill=gray!10] (empty) {};
    \draw node [draw, minimum width = 1cm, minimum height = 1cm, node distance = 1cm, below of=0, fill=gray!10] {};
    \draw node [draw, minimum width = 1cm, minimum height = 1cm, node distance = 1cm, below of=empty, fill=gray!10] {$#2$};
    %\draw grid[very thick] (2,2);
    %\draw node (L) at (0.5,1.5) {$0$};
    %\draw node (H) at (1.5,0.5) {$#1^\sim.H$};
  \end{tikzpicture}
}
\tikzstyle{line} = [draw, -latex']
\begin{tikzpicture}[auto, >=triangle 45]

  \draw
  node [block] (s1) {$s_1$}
  node [block, right of=s1] (s2) {$s_2$}
  node [right of=s2, node distance=1.8cm] (cdots) {$\cdots$}
  node [block, right of=s2] (sN) {$s_N$}
  node [below of=s1, node distance=2.8cm] (DWT1) {\DWT{\scriptstyle s_1.L}{\scriptstyle s_1.H}}
  node [below of=s2, node distance=2.8cm] (DWT2) {\DWT{\scriptstyle s_2.L}{\scriptstyle s_2.H}}
  node [below of=sN, node distance=2.8cm] (DWTN) {\DWT{\scriptstyle s_N.L}{\scriptstyle s_N.H}}
  node [below of=DWT1, node distance=1.4cm] (S0) {$S_1$}
  node [below of=DWT2, node distance=1.4cm] (S0) {$S_2$}
  node [below of=DWTN, node distance=1.4cm] (SN) {$S_N$}
  ;

  \draw [->] (s1) -- (DWT1);
  \draw [->] (s2) -- (DWT2);
  \draw [->] (sN) -- (DWTN);

\end{tikzpicture}
\end{document}
