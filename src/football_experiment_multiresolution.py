...
for k in range(config.n_frames):
    ...
    decomposition = DWT.analyze(V_k, n_levels=config.levels)
    #print("len =", len(decomposition))
    L.write(decomposition[0], f"{config.input_video}{config.levels}_", k)
    for l in range(0, config.levels):
        H.write(decomposition[l+1], f"{config.input_video}{config.levels-l}_", k)

print("IPP... encoding")

print(f"Computing SRL {config.levels}")
IPP_step.encode(f"{config.input_video}{config.levels}_", f"{config.codestream}{config.levels}_", config.n_frames, config.q_step)
for k in range(config.n_frames):
    reconstructed__V_k_H = L.read(f"{config.input_video}{config.levels}_reconstructed_H", k)
    V_k_L = L.read(f"{config.input_video}{config.levels}_", k)
    reconstructed_V_k_H = H.reduce(reconstructed__V_k_H)
    reconstructed_V_k = DWT.synthesize_step(V_k_L, reconstructed_V_k_H)
    L.write(reconstructed_V_k, f"{config.input_video}{config.levels-1}_", k)

for l in range(config.levels-1, 0, -1):
...
for l in range(config.levels, -1, -1):
    for k in range(config.n_frames):
