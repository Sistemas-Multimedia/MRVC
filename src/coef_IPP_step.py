...
def encode(video, n_frames, q_step):
    try:
        ...
        for k in range(1, n_frames):
            ...
            frame.debug_write(values.norm(prediction__V_k_L)[0], f"{video}encoder_prediction_L_", k)
            _V_k_1_L = _V_k_L # (E.b)
            _E_k_L = _V_k_L - prediction__V_k_L # (E.e)
            frame.debug_write(values.norm(_V_k_L)[0], f"{video}encoder_predicted_L_", k)
