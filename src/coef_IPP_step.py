...
def encode(video, n_frames, q_step):
    try:
        ...
        dequantized__E_k_H = codec.E(_E_k_H, video, 0, q_step)
        ...
        for k in range(1, n_frames):
            ...
            frame.debug_write(values.normalize(prediction__V_k_L)[0], f"{video}encoder_prediction_L_", k)
            _V_k_1_L = _V_k_L # (E.b)
            _E_k_L = _V_k_L - prediction__V_k_L # (E.e)
            frame.debug_write(values.normalize(_V_k_L)[0], f"{video}encoder_predicted_L_", k)
            frame.debug_write(values.clip(_E_k_L), f"{video}encoder_prediction_error_L_", k)
            ...
            _V_k_H = H.interpolate(V_k_H) # (b)
            frame.debug_write(values.clip(_V_k_H), f"{video}encoder_predicted_H_", k)
            prediction__V_k_H = motion.make_prediction(reconstructed__V_k_1_H, flow) # (E.j)
            frame.debug_write(values.clip(prediction__V_k_H), f"{video}encoder_prediction_H_", k)
            ...
            frame.debug_write(values.clip(IP_prediction__V_k_H), f"{video}encoder_IP_prediction_H_", k)
            ...
            frame.debug_write(values.clip(_E_k_H), f"{video}encoder_prediction_error_H_", k)
            #quantized__E_k_H = Q.quantize(_E_k_H, step=q_step) # (d)
            #dequantized__E_k_H = Q.dequantize(quantized__E_k_H, step=q_step) # (E.g)
            dequantized__E_k_H = codec.E(_E_k_H, video, k, q_step)
            ...
            frame.debug_write(values.clip(dequantized__E_k_H), f"{video}encoder_dequantized_prediction_error_H_", k)
            ...
            frame.debug_write(values.clip(reconstructed__V_k_H), f"{video}encoder_reconstructed_", k)
            reconstructed__V_k_1_H = reconstructed__V_k_H # (E.i)
            #quantized_E_k_H = H.reduce(quantized__E_k_H) # (f)
            #L.write(YCoCg.to_RGB(V_k_L), codestream, k) # (g)
            #L.write(V_k_L, video, k) # (g)
