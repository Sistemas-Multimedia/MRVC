...
def encode(input_seq=INPUT_SEQ, output_seq=OUTPUT_SEQ, n_frames=5):
    ...
    V_k = frame.read(input_seq, ASCII_k)
    ...
    #print(quantized_E_k_H[0].shape)
    write_L(V_k_L, output_seq, ASCII_k) # (p)
    write_H(quantized_E_k_H, output_seq, ASCII_k) # (p)
    for k in range(1, n_frames):
        ...
        frame.write(reconstructed__V_k_H + 128, output_seq + "reconstructed_encoder_", ASCII_k)
        reconstructed__V_k_1_H = reconstructed__V_k_H # (m)

        quantized_E_k_H = reduce_H(quantized__E_k_H) # (o)
        #print(quantized_E_k_H[0].shape)
        write_L(V_k_L, "/tmp/", ASCII_k) # (p)
        write_H(quantized_E_k_H, "/tmp/", ASCII_k) # (p)
    
def decode(input_seq=INPUT_SEQ, output_seq=OUTPUT_SEQ, n_frames=5):
    k = 0
    V_k_L = read_L(input_seq, ASCII_k) # (q)
    quantized_E_k_H = read_H(input_seq, ASCII_k) # (q)
    ...
    frame.write(reconstructed_V_k, output_seq, ASCII_k)
    for k in range(1, n_frames):
        ASCII_k = str(k).zfill(3)
        V_k_L = read_L(input_seq, ASCII_k) # (q)
        quantized_E_k_H = read_H(input_seq, ASCII_k) # (q)
        ...
        reconstructed_V_k = DWT.synthesize(V_k_L, reconstructed_V_k_H) # (t)
        reconstructed_V_k = YCoCg.to_RGB(reconstructed_V_k)
        frame.write(reconstructed_V_k, output_seq + "reconstructed_decoder", ASCII_k)
