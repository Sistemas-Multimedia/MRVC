...
def encode(video=VIDEO_PREFIX, codestream=CODESTREAM_PREFIX, n_frames=N_FRAMES, q_step=Q_STEP):
    try:
        ...
        for k in range(1, n_frames):
            ...
            frame.debug_write(prediction__V_k_L, f"{codestream}encoder_prediction_L_{k:03d}")
            _V_k_1_L = _V_k_L # (E.b)
            _E_k_L = _V_k_L - prediction__V_k_L # (E.e)
            frame.debug_write(_V_k_L, f"{codestream}encoder_predicted_L_{k:03d}")
            S_k = _E_k_L < _V_k_L # (E.f)
            if __debug__:
                unique, counts = np.unique(S_k, return_counts=True)
                print(unique, counts)
            frame.debug_write(S_k, f"{codestream}encoder_selection_{k:03d}")
            _V_k_H = H.interpolate(V_k_H) # (b)
            frame.debug_write(_V_k_H + 128, f"{codestream}encoder_predicted_H_{k:03d}")
            prediction__V_k_H = motion.make_prediction(reconstructed__V_k_1_H, flow) # (E.j)
            frame.debug_write(prediction__V_k_H + 128, f"{codestream}encoder_prediction_H_{k:03d}")
            IP_prediction__V_k_H = np.where(S_k, prediction__V_k_H, 0) # (E.k)
            #IP_prediction__V_k_H = np.zeros_like(S_k) # (E.k)
            frame.debug_write(IP_prediction__V_k_H + 128, f"{codestream}encoder_IP_prediction_H_{k:03d}")
