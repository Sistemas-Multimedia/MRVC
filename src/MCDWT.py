        ...
        for k in range(K):  # spatial scale
            ...
            while x < N:
                ...
                while i < (N // x):
                    ...
                    if __debug__:
                        image.write(CL, "{}{:03d}_{}".format(
                            prefix + "_CL_", x * i + x, k))
                    CH = dwt.backward(zero_L, dwtC[1])
                    if __debug__:
                        image.write(CH, "{}{:03d}_{}".format(
                            prefix + "_CH_", x * i + x, k))
                    ...
                    if __debug__:
                        ...
                        image.write(prediction, "{}{:03d}_{}".format(
                            prefix + "_prediction_L_", x * i + x // 2, k))
                    ...
                    rBH = BH - prediction
                    if __debug__:
                        image.write(rBH, "{}{:03d}_{}".format(
                            prefix + "_residue_", x * i + x // 2, k))
                    rBH = dwt.forward(rBH)
                    rBH[0][0:L_y, 0:L_x, :] = dwtB[0]
                    decomposition.write(rBH, "{}{:03d}_{}".format(
                        prefix, x * i + x // 2, k + 1))
                    AL = CL
                    AH = CH
                    i += 1
...
#        "  yes | cp -rf ../sequences/stockholm/ /tmp/\n"
    ...
    if args.backward:
        ...
        p = d.forward(args.decompositions,
                      args.mc_decompositions, args.N, args.T)
