    ...
    def forward(self, prefix="/tmp/", N=5, T=2):
        ...
        for t in range(T):  # Temporal scale
            ...
            while i < (N // x):
                ...
                decomposition.writeH(
                    bH, prefix, "{:03d}".format(x * i + x // 2))
                aL, aH = cL, cH
                i += 1
            x *= 2
...
#        "  yes | cp -rf ../sequences/stockholm/ /tmp/\n"
    ...
    parser.add_argument(
        "-p", "--prefix", help="Dir where the files the I/O files are placed", default="/tmp/")
    parser.add_argument(
        "-N", help="Number of decompositions", default=5, type=int)
    parser.add_argument(
        "-T", help="Number of temporal levels", default=2, type=int)
    parser.add_argument(
        "-P", help="Predictor to use (1=average, 2=weighted_average)", default=1, type=int)
