...
class image_IPP_class():
    ...
    def create_structures(self, W_k):
        global initial_flow
        initial_flow = np.zeros((W_k.shape[0], W_k.shape[1], 2), dtype=np.float32)

    def compute_averages(self, V_k):
        pass

    def decide_types(self, video, k, q_step, V_k, reconstructed_V_k, E_k, prediction_V_k):
    ...
    def encode(self,
        ...
        try:
            ...
            for k in range(1, n_frames):
                ...
                frame.debug_write(self.clip(YUV.to_RGB(prediction_V_k)), f"{video}encoder_prediction", k)
                E_k = V_k - prediction_V_k[:V_k.shape[0], :V_k.shape[1], :] # (f)
                frame.debug_write(self.clip(YUV.to_RGB(E_k)+128), f"{video}encoder_prediction_error", k)
                ...
                frame.debug_write(self.clip(YUV.to_RGB(dequantized_E_k)), f"{video}encoder_dequantized_prediction_error", k)
                ...
                reconstructed_V_k = self.decide_types(video, k, q_step, V_k, reconstructed_V_k, E_k, prediction_V_k)
                frame.debug_write(self.clip(YUV.to_RGB(reconstructed_V_k)), f"{video}reconstructed_", k) # Decoder's output
                reconstructed_V_k_1 = reconstructed_V_k # (j)
        except:
...
encoder = image_IPP_class()
def encode(video, n_frames, q_step):
    encoder.encode(video, n_frames, q_step)

def compute_br(prefix, frames_per_second, frame_shape, n_frames):
    return encoder.compute_br(prefix, frames_per_second, frame_shape, n_frames)
