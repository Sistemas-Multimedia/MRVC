...
''' MRVC/quantize image sequence compressor. '''
...
parser.add_argument("-q", "--quantization_step", type=int, help"Quantization step size")
    
class Quantizer():
    def __init__(self):
        if __debug__:
            print("Quantize.__init__")
        else:
            pass

    def encode(self, x, q_step):
        k = (x / quantization_step).astype(np.int16)
            return k

    def decode(self, k, q_step):
        y = quantization_step * k
            return y

    def (self, chunk_number, chunk):
